import { deleteToken, initializeFirebase, setup } from "./fcm";
...
export function initRaven(ravenAppId, ravenSecretKey) {
...
export function setupPushNotification(onError, onTokenReceived) {
  setup(onError, (token) => {
  ...
  try {
    ...
    if (payload && payload["type"] === "DELIVERED") {
      navigator.serviceWorker.getRegistration().then(function (reg) {
        ...
        setTimeout(() => {
          api.updateStatus(payload["data"]["raven_notification_id"], "DELIVERED");
        }, 2000);
      });
    }
